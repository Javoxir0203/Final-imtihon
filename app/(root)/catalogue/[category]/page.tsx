      ...
      <div className="container mx-auto my-12 flex gap-4">
        <div className="flex h-max flex-col gap-3 rounded border border-gray-500/20 p-4">
          ...
          {catalogue.map((item, index) => (
            <Link href={`/catalogue/${item.slug}`} key={index} className={cn("flex items-center justify-between gap-8 rounded-md px-4 py-2 transition-colors hover:bg-gray-500 hover:text-white", category === item.slug && "bg-gray-500 font-bold text-white")}>
              <p className="font-light">{item.title}</p>
              {item.slug === category ? <ChevronDown className={"text-white"} /> : <ChevronRight className={""} />}
            </Link>
          ))}
        </div>
